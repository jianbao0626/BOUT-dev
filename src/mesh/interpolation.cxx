...
/*!
  ...
  NOTE: Since corner guard cells cannot be communicated, it never makes sense
  to calculate interpolation in guard cells. If guard cell values are required,
  we must communicate. Since mesh->communicate() communicates both x- and
  y-guard cells by default, there is no difference between RGN_ALL, RGN_NOX and
  RGN_NOY.
...
const Field2D interp_to(const Field2D &var, CELL_LOC loc, REGION region) {
  // Note: currently Field2D variables can only have location=CELL_CENTRE
  // we do not set the location of the result, and it should only be used as a
  // temporary variable, because the location given by getLocation() will be
  // inconsistent
  ...
  if (fieldmesh->StaggerGrids && (var.getLocation() != loc)) {
    ...
    // setLocation should be implemented for Field2D and enabled here in future
    //result.setLocation(loc);
