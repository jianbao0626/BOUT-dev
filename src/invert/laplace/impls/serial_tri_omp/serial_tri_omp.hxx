...
class LaplaceSerialTriOmp;

#ifndef __SERIAL_TRIOMP_H__
#define __SERIAL_TRIOMP_H__

#include <invert_laplace.hxx>
#include <dcomplex.hxx>
#include <options.hxx>

class LaplaceSerialTriOmp : public Laplacian {
public:
  LaplaceSerialTriOmp(Options *opt=NULL);
  ~LaplaceSerialTriOmp();
  ...
  void setCoefEx(const Field2D &UNUSED(val)) override {
    throw BoutException("LaplaceSerialTriOmp does not have Ex coefficient");
  }
  using Laplacian::setCoefEz;
  void setCoefEz(const Field2D &UNUSED(val)) override {
    throw BoutException("LaplaceSerialTriOmp does not have Ez coefficient");
  }
...
private:
   ...
   *        LaplaceSerialTriOmp::solve()
   * bk1d = The 1d array of bk
   * xk   = The fourier transformed of x, where x the output of
   *        LaplaceSerialTriOmp::solve()
   * xk1d = The 1d array of xk
   */
  typedef std::vector<std::vector<dcomplex>> dcomplexMatrix;
  dcomplexMatrix xk, bk;
  //  Array<dcomplex> xk1d, bk1d;
   ...
   *       above
   */
  //  Array<dcomplex> avec, bvec, cvec;
};

#endif // __SERIAL_TRIOMP_H__
