...
GRID_SIZES=(4 8 16 32 64 128 256 512)
EXE=iterator-offsets
NP=1
FLAGS="-q -q -q -q performanceIterator:profileMode=true performanceIterator:NUM_LOOPS=1000"
...
do
    mpirun -bind-to socket -np ${NP} ./${EXE} ${FLAGS} mesh:nx=${currGrid} mesh:ny=${currGrid} mesh:nz=${currGrid} performanceIterator:includeHeader=false
done
